...
from django.db.models import F, Sum, Avg, ExpressionWrapper
...
class LoginView(FormView):
    ...
    def get(self, request):
        if request.user.is_authenticated:
            ...
            if request.session.has_key('registered_mobile'):
                ...
                user = authenticate(
                    request, employee_number=mobile, password=password)
    ...
    def form_valid(self, form):
        ...
        if request.session.has_key('registered_mobile'):
            ...
            user = authenticate(
                request, employee_number=employee_number, password=password)
            if user is not None and user.role != "Super Admin" and user.is_active and not user.is_deleted:
                login(request, user)
                ...
                if is_safe_url(next, allowed_hosts=None):
                    ...
                    if SiteUser.objects.get(id=request.user.id).is_deleted == True:
                        messages.error(
                            request, "Deleted user cannot be logged in !")
...
def create_admin(request):
    ...
    if request.method == 'POST' or request.method == 'FILES':
        ...
        professional_email_password = request.POST.get(
...
def manager_list(request):
    if request.user.role == 'Super Admin':
        manager_list = SiteUser.objects.filter(
            role='Manager', group__icontains=request.user.name, is_deleted=False)
    else:
        manager_list = SiteUser.objects.filter(
            role='Manager', admin__icontains=request.user.name, is_deleted=False)
    context = {
...
def employee_list(request):
    if request.user.role == 'Super Admin':
        employee_list = SiteUser.objects.filter(
            role='Employee', group__icontains=request.user.name, is_deleted=False)
    elif request.user.role == 'Admin':
        employee_list = SiteUser.objects.filter(
            role='Employee', admin__icontains=request.user.name, is_deleted=False)
    elif request.user.role == 'Manager':
        employee_list = SiteUser.objects.filter(
            role='Employee', manager__icontains=request.user.name, is_deleted=False)
    context = {
...
def create_employee(request):
    ...
    if SiteUser.objects.get(id=request.user.pk).role == 'Admin':
        ...
        admin_logged = SiteUser.objects.get(
            id=request.user.pk, is_deleted=False).admin
        is_admin = False
            ...
            for item in afd:
                ...
                if (item.follow_up_history.is_email):
                    if (item.follow_up_history.html_content != None and item.follow_up_history.html_content != '' and len(item.follow_up_history.html_content) > 5):
                        ...
                        send_html_mail(item.follow_up_history.email_subject, item.follow_up_history.html_content, settings.EMAIL_HOST_USER2, [
                ...
                if (item.follow_up_history.is_sms):
                    ...
                    url = "http://smshorizon.co.in/api/sendsms.php?user=" + settings.user + "&apikey=" + settings.api + "&mobile=" + item.follow_up_history.follow_up_section.lead_id.customer_id.contact_no + \
                        "&message=" + item.follow_up_history.sms_msg+'\n' + \
                ...
                Auto_followup_details.objects.filter(id=item.id).update(
                    followup_date=end_date, no_of_times_fdone=F('no_of_times_fdone')+1)
                Lead.objects.filter(id=item.follow_up_history.follow_up_section.lead_id).update(
                    no_of_times_followup_done=F('no_of_times_followup_done')+1)
        except Exception as e:
...
@login_required(login_url='/')
def profit_calculation(request):
    todays_date = datetime.now().date()
    ...
    print(current_month)
    print(current_year)
    # print(Purchase_Details.objects.filter(
    #     date_of_purchase__month=current_month, date_of_purchase__year=current_year).count())
    ...
    # print('purchase count-->', purchase_details.count())
    ...
    current_month_gross_date = []
    current_month_gross = []

    # Initialize variables to store the daily sums
    daily_sums = {}

    # Iterate through the queryset and calculate the daily sums
    for purchase_detail in purchase_details:
        try:
            date_of_purchase = purchase_detail.date_of_purchase
            daily_sums[date_of_purchase] = daily_sums.get(
                date_of_purchase, 0.0)

            # Calculate the daily sum of value of goods
            daily_sums[date_of_purchase] += float(
                purchase_detail.value_of_goods)
            print(
                '<-----------------------------single purchase detail-------------------->')
            print('date-->', date_of_purchase)
            print('customer name--->', purchase_detail.id)
            print('\npurchase detail amount --->',
                  str(purchase_detail.value_of_goods))

            # Subtract the daily sum of total cost price
            for product_detail in purchase_detail.product_details.all():
                daily_sums[date_of_purchase] -= (
                    float(product_detail.cost_price) * float(product_detail.quantity))
                print('product detail --->', str(product_detail.cost_price), str(product_detail.quantity),
                      str(float(product_detail.cost_price) * float(product_detail.quantity)))
        except Exception as e:
            print(e)

    # Iterate through the daily sums and populate the lists
    for date, gross_profit in daily_sums.items():
        current_month_gross_date.append(date.strftime('%Y-%m-%d'))
        current_month_gross.append(gross_profit)

    print('current_month_gross_date', current_month_gross_date)
    print('curren_month_gross', current_month_gross)
    # print(' gross details', previous_month)
    # print(daily_sums)
    # result = purchase_details.order_by('date_of_purchase')\
    #     .values('date_of_purchase') \
    #     .annotate(
    #     sum_of_val_of_goods=Sum('value_of_goods', output_field=FloatField()),
    #     sum_of_product_details_amount=ExpressionWrapper(
    #         F('product_details__quantity') *
    #         F('product_details__cost_price'),
    #         output_field=FloatField()
    #     ),
    # ).order_by('date_of_purchase')

    # result = Purchase_Details.objects.filter(
    #     date_of_purchase__month=current_month,
    #     date_of_purchase__year=current_year
    # ).values('date_of_purchase').annotate(
    #     sum_of_val_of_goods=Cast(Sum('value_of_goods'), FloatField()),
    #     sum_of_product_details_amount=Cast(
    #         'product_details__godown_id__godown_products__product_id__cost_price', FloatField())
    # sum_of_product_details_amount=Cast(
    #     Sum(Cast('product_details__quantity', FloatField()) *
    #         Cast('product_details__godown_id__godown_products__product_id__cost_price', FloatField())),
    #     FloatField()
    # ),
    # gross_profit=Cast(
    #     Sum('value_of_goods') - Sum(Cast('product_details__quantity', FloatField()) *
    #                                 Cast('product_details__godown_id__godown_products__product_id__cost_price', FloatField())),
    #     FloatField()
    # ),
    # ).order_by('date_of_purchase')
    ...
    # purchase_details.order_by('date_of_purchase')\
    #         .values('date_of_purchase') \
    #         .annotate(sum_of_val_of_goods=Cast(Sum('value_of_goods'), FloatField()),
    #                   sum_of_product_details__amount=Cast(Sum('product_details__amount'), FloatField()),
    #                   data_sum=Cast(Sum('value_of_goods'), FloatField()) - Cast(Sum('product_details__amount'), FloatField()))
    # for purchase in purchase_details:
    #     for product_detail in purchase.product_details.all():
    #         print( product_detail )

    #         print( product_detail.quantity )
    #         print( product_detail.cost_price )
    #         cost_price = sum(
    #             sum(product_detail.quantity * product_detail.cost_price for product_detail in purchase.product_details.all())
    #             for purchase in purchase_details
    #         )
    # print('cost price-->')
    # print(cost_price)
    ...
    # datetime.now().year
    # 1. Sales for current month on daily bais.
    # this_month = Employee_Analysis_date.objects.filter(user_id=user_id,entry_date__month=mon).values('entry_date',
    #                                                                                                      'total_sales_done_today').order_by('entry_date')
    if request.user.role == "Super Admin":
        this_month = Purchase_Details.objects.filter(
            date_of_purchase__month=current_month, date_of_purchase__year=current_year)\
            .order_by('date_of_purchase')\
            .values('date_of_purchase') \
            .annotate(data_sum=Cast(Sum('value_of_goods'), FloatField()))
    elif request.user.role == "Admin" or request.user.role == "Manager":
        this_month = Purchase_Details.objects.filter(Q(user_id__name=request.user.name) | Q(user_id__group__icontains=request.user.name),
                                                     date_of_purchase__month=current_month, date_of_purchase__year=current_year).order_by('date_of_purchase')\
            .values('date_of_purchase').annotate(data_sum=Cast(Sum('value_of_goods'), FloatField()))
    else:  # for employee
        this_month = Purchase_Details.objects.filter(sales_person=SiteUser.objects.get(id=request.user.id).profile_name,
                                                     date_of_purchase__month=current_month, date_of_purchase__year=current_year).order_by('date_of_purchase')\
            .values('date_of_purchase').annotate(data_sum=Cast(Sum('value_of_goods'), FloatField()))
    current_month_sales_date = []
    current_month_sales = []
    for i in this_month:
        x = i
        current_month_sales_date.append(
            x['date_of_purchase'].strftime('%Y-%m-%d'))
        current_month_sales.append(x['data_sum'])

    print('current month sales', current_month_sales)
    print('current month date -->', current_month_sales_date)
    print(' sales details', this_month)

    # 2. Gross profit of current month on daily basis.
    ...
    if request.user.role == "Super Admin":
        previous_month = Purchase_Details.objects.filter(
            ...
            .values('date_of_purchase') \
            .annotate(sum_of_val_of_goods=Cast(Sum('value_of_goods'), FloatField()),
                      sum_of_product_details__amount=Cast(
                          Sum('product_details__amount'), FloatField()),
                      data_sum=Cast(Sum('value_of_goods'), FloatField()) - Cast(Sum('product_details__amount'), FloatField()))
    elif request.user.role == "Admin" or request.user.role == "Manager":
        previous_month = Purchase_Details.objects.filter(Q(user_id__name=request.user.name) | Q(user_id__group__icontains=request.user.name),
                                                         date_of_purchase__month=current_month, date_of_purchase__year=current_year).order_by('date_of_purchase')\
    ...
    # for i in previous_month:
    #     x = i
    #     current_month_gross_date.append(
    #         x['date_of_purchase'].strftime('%Y-%m-%d'))
    #     current_month_gross.append(x['data_sum'])
    ...
    # # Iterate through the queryset and calculate the monthly sums
    # for purchase_detail in purchase_details:
    #     month_year_key = (purchase_detail.date_of_purchase.month,
    #                       purchase_detail.date_of_purchase.year)
    #     monthly_sums[month_year_key] = monthly_sums.get(month_year_key, 0.0)

    #     # Calculate the monthly sum of value of goods
    #     monthly_sums[month_year_key] += float(purchase_detail.value_of_goods)

    #     # Subtract the monthly sum of total cost price
    #     for product_detail in purchase_detail.product_details.all():
    #         monthly_sums[month_year_key] -= (
    #             float(product_detail.cost_price) *
    #             float(product_detail.quantity)
    #         )

    # # Initialize lists to store dates and corresponding gross profits
    # monthly_gross_date = []
    # monthly_gross = []

    # # Iterate through the monthly sums and populate the lists
    # for (month, year), gross_profit in monthly_sums.items():
    #     monthly_gross_date.append(f"{year}-{month:02d}")
    #     monthly_gross.append(gross_profit)
    ...
    if request.method == 'POST' or request.method == 'FILES':
        ...
        if 'mobile' in request.POST or request.method == 'FILES':
            ...
            if product_master == 'True':
                ...
                item.product_master_access = False
            item.save(update_fields=['product_master_access', 'professional_email_password', 'professional_email', 'pancard', 'aadhar_card', 'password_text', 'employee_number',
                      'mobile', 'email', 'profile_name', 'role', 'group', 'is_deleted', 'modules_assigned', 'bank_name', 'account_number', 'branch_name', 'ifsc_code', 'password'])
            return redirect('/manager_list/')
    context = {
        'form': form,
    ...
    if request.method == 'POST' or request.method == 'FILES':
        ...
        if 'mobile' in request.POST or request.method == 'FILES':
            ...
            if upload_pancard != None and upload_pancard != "":
                item.upload_pancard = upload_pancard
                item.save(update_fields=['upload_pancard', ])
            item.save(update_fields=['professional_email_password', 'product_master_access', 'professional_email', 'pancard', 'aadhar_card', 'password_text', 'mobile', 'email',
                      'employee_number', 'profile_name', 'role', 'group', 'is_deleted', 'modules_assigned', 'bank_name', 'account_number', 'branch_name', 'ifsc_code', 'password'])
            return redirect('/employee_list/')
    context = {
        'form': form,
...
def forgotpassword(request):
    ...
    if request.method == 'POST':
        ...
        send_mail('HSCO: Your Password has been reset successfully!!!',
                  'New Password: '+password, settings.EMAIL_HOST_USER, [email])
