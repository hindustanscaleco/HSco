...
@receiver(pre_save, sender=Amc_After_Sales)
def amc_handler(sender, instance, update_fields=None, **kwargs):
    try:
...
def amc_views(request):
    if request.method == 'POST':
        ...
        if check_admin_roles(request):  # For ADMIN
            amc_list = Amc_After_Sales.objects.filter(Q(user_id__name=request.user.name)|Q(user_id__group__icontains=request.user.name),
