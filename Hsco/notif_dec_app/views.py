...
def notif_decl_home(request):
    if request.user.role == 'Super Admin':
        leave_req_list = Employee_Leave.objects.filter(user_id__group__icontains=request.user.name,
                                                       user_id__is_deleted=False)
        warn_def_list = Defects_Warning.objects.filter(user_id__group__icontains=request.user.name,
                                                       user_id__is_deleted=False)
    else:
...
def chat_details(request, from_id, to_id):
    ...
    Chat_model.objects.filter(Q(message_to=SiteUser.objects.get(id=from_id).id) & Q(is_viewed=False)).update(
        is_viewed=True)
...
def chat_with_user(request):
    if request.user.role == 'Employee':
        ...
        all_users_list = SiteUser.objects.filter(
            Q(name=SiteUser.objects.get(id=request.user.id).manager) | Q(role='Super Admin'))
    elif request.user.role == 'Manager':
        all_users_list = SiteUser.objects.filter(
            Q(name=SiteUser.objects.get(id=request.user.id).admin) | Q(manager=request.user.name) | Q(
                role='Super Admin'))
...
def notification_context(request):
    if request.user.is_authenticated:
        ...
        message = Chat_model.objects.filter(message_to=request.user.id, is_viewed=False, is_warning=False,
                                            is_defect=False)
        alert = Chat_model.objects.filter(
            (Q(message_to=request.user.id) & Q(is_viewed=False)) & (Q(is_warning=True) | Q(is_defect=True)))
        postponed_alert = Lead.objects.filter(
            Q(current_stage='Postponed') & Q(postpond_time_date__gte=datetime.date.today()) & Q(
                owner_of_opportunity__id=request.user.pk))
        # next_date = datetime.date.today() + timedelta(days=1)

        critical_limit_notif = GodownProduct.objects.filter(critical_limit__gte=F('quantity'),
                                                            godown_id__goddown_assign_to=request.user)
        request_admin_list = GoodsRequest.objects.filter(
            Q(request_admin=True) & Q(req_from_godown__godown_admin__id=request.user.id)) | \
                             GoodsRequest.objects.filter(
                                 Q(request_admin=True) & Q(request_admin_id__id=request.user.id))
        ...
        if request.user.role == 'Super Admin':
            req_product_mismatch_notif = RequestedProducts.objects.filter(
                Q(goods_req_id__notify=True) & (~Q(sent_quantity=F('received_quantity') + F('faulty_quantity')) | ~Q(
                    sent_carton_count=F('received_carton_count') + F('received_carton_count')))).order_by('-id')
        else:
            req_product_mismatch_notif = RequestedProducts.objects.filter(
                (Q(goods_req_id__notify=True) & ~Q(sent_quantity=(F('received_quantity') + F('faulty_quantity'))) | ~Q(
                    sent_carton_count=(F('received_carton_count') + F('received_carton_count')))) & Q(
                    godown_id__godown_admin=request.user)
            ).order_by('-id')
