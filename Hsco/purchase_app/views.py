...
def view_customer_details(request):
    ...
    if request.method == 'POST' and 'deleted' not in request.POST:
        ...
        if check_admin_roles(request):  # For ADMIN
            cust_list = Purchase_Details.objects.filter(user_id__name=request.user.name,user_id__group__icontains=request.user.name,user_id__is_deleted=True,user_id__modules_assigned__icontains='Customer Module').order_by('-id')
        else:  # For EMPLOYEE
